...
#            res=x['fu_lastRecord']
    ...
    sTime=encounterSummary(pd.merge(fev,eList,on='PAT_ENC_CSN_ID',how='left'))
    sTime.set_index('PAT_ID',inplace=True)
    
    ########## Date (time) of the last time each patient was observed by the health system
    sTime=sTime.join(eList.groupby('PAT_ID')['EVENT_TIME'].max().rename('maxTime'))
    sTime=sTime.join(eList.groupby('PAT_ID')['EVENT_TIME'].min().rename('minTime'))
