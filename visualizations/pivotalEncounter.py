...
def pivotalEncounter(evt,M,colName='LABEL',wdw=365.25/2,buffer=1,flt=None,countPatients=True):
    ...
    kp=(M['PAT_ENC_CSN_ID']!=M['eID']) & ((M['EVENT_TIME']<M['eStartTime']-q) & (M['EVENT_TIME']>M['eStartTime']-w))
    u=M.loc[kp,['PAT_ID',colName]]
    if countPatients:
        u=u.drop_duplicates()
    v=pd.DataFrame(u[colName].value_counts().rename(beforeLbl))
    
    kp=(M['PAT_ENC_CSN_ID']==M['eID']) | ((M['EVENT_TIME']>=M['eStartTime']-q) & (M['EVENT_TIME']<=M['eStartTime']+q))
    u=M.loc[kp,['PAT_ID',colName]]
    if countPatients:
        u=u.drop_duplicates()
    v=pd.merge(v,pd.DataFrame(u[colName].value_counts().rename(durLbl)),how='outer',left_index=True,right_index=True)
    
    kp=(M['PAT_ENC_CSN_ID']!=M['eID']) & ((M['EVENT_TIME']>M['eEndTime']+q) & (M['EVENT_TIME']<M['eEndTime']+w))
    u=M.loc[kp,['PAT_ID',colName]]
    if countPatients:
        u=u.drop_duplicates()
    ...
    if flt is None:
        flt=min(.005*len(set(M['PAT_ID'])),10)
